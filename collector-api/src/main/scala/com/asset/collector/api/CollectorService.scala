...
trait CollectorService extends Service{
  ...
  def filterStock: ServiceCall[NotUsed, Done]
  def filterStock1: ServiceCall[NotUsed, Done]
  def filterStock2: ServiceCall[NotUsed, Done]
  ...
  override def descriptor: Descriptor ={
    ...
    named("Collector")
      .withCalls(
        ...
        restCall(Method.POST, "/stock/korea/indexes/filter", filterStock),
        restCall(Method.POST, "/stock/korea/indexes/filter1", filterStock1),
        restCall(Method.POST, "/stock/korea/indexes/filter2", filterStock2),
      ).withAutoAcl(true)
